...
public class MusicPlayer extends PlaybackListener {
    //used to update ispaused more accurately
    private static final Object playSignal = new Object();

    //need reference so that we can update the gui in this class
    private MusicPlayerGUI musicPlayerGUI;

    //a way to store song's details, so creating a song class
    ...
    //we will need to keep tracking the index we are in the playlist
    private int currentPlaylistIndex;

    //use JLayer library to create an advanced player obj which will handle playing the music
    private AdvancedPlayer advancedPlayer;

    //pause boolean flag used to indicate whether the player has been paused
    private boolean isPaused;

    //boolean flag used to tell when the song has fininshed
    private boolean songFinished;

    private boolean pressedNext, pressedPrev;

    //stores in the last frame when the playback is finished(for pausing and resuming)
    ...
    public void loadPlayList(File playlistFile){
        ...
        try{
            ...
            //reach each line from text file and store the text into the songpath variable
            String songPath;
    ...
    public void nextSong() {
        ...
        //check to see if we reached the end of the playlist. if so then don't do anything
    ...
    public void prevSong(){
        ...
        //check to see if we can go the previous song
                ...
                try {
                    if (isPaused) {
                        synchronized (playSignal) {
                            ...
                            //notify th eother thread to continue (,makes sure that ispaused is updated to false properly)
                            playSignal.notify();
                        }
            ...
            public void run() {
                if (isPaused) {
                    try {
                        ...
                        //makes sure that ispused boolean flag updates to false before continuing
                        synchronized (playSignal) {
                            playSignal.wait();
                        }
                    }catch (Exception e){
    ...
    public void playbackFinished(PlaybackEvent evt) {
        //this method is called when the song finishes or if the player gets closed
        System.out.println("Playback Finished");
        if(isPaused) {
            currentFrame += (int) ((double) evt.getFrame() * currentSong.getFrameRatePerMilliseconds());
        }else{
            //if the user pressed next or prev, we dont need to execute the rst of the code
